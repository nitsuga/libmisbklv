...
- **Teardown ordering is load-bearing when a probe is attached.** Under
  ...
  sever those probes and reach `GST_STATE_NULL` before `VideoCtx` is freed, or
  the streaming thread races the free (parrot-to-klv#57). See 0024 § *Teardown:
  SEI probe lifetime*.
