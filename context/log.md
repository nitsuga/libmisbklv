...
* **chore: make C++ formatting reproducible.** Added a root `.clang-format` based on LLVM's C++ style with the repository's established differences explicit: 2-space structure with indented access/case labels, left-bound pointers, 100-column lines, two spaces before trailing comments, preserved include order, and one-line guards/loops. The agent guide uses `git clang-format origin/main` so formatting stays limited to branch-modified lines, and explicitly rejects unrelated whole-file/tree sweeps; the current source predates a formatter baseline, so this change defines future formatting without pretending every existing file already conforms.
