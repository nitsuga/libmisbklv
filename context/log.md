...
* **bench: add `klv_bench`, and let it settle the parked performance questions** (issue #50). A timing harness over the codec, `Message`, `extract_ts_klv`, and gst-extraction paths — median of repeated runs, no dependency beyond `<chrono>`, built by default so it cannot rot, and deliberately **not** a CTest case, since a timing threshold in CI measures the runner rather than the library. The `.ts` fixtures are ~1.5 KiB, so the extraction rows concatenate whole copies to a requested size (valid for a transport stream, and avoids committing a large binary). **Writing it surfaced two defects in the harness itself, both found by running it rather than reading it:** the per-kind codec loop filtered on `!variable`, which excluded all eighteen IMAPB items in the 0601 registry — every one is variable-width with a `fixed_len` default — so the single row the benchmark existed to produce was silently absent; and the edited-encode row called `set()` on a tag whose kind did not match the value, so it failed with `TypeMismatch` and skipped without a word. Both now report rather than vanish. **What the numbers say, on one desktop, with codec rows moving ~15% run to run:** IMAPB decode+encode is ~190–220 ns against ~80–92 ns for LinearLDS, the closest same-shape control, so precomputing the ST 1201 scaling factors ([#49](https://github.com/nitsuga/libmisbklv/issues/49)) would save on the order of 130 ns per item — roughly 0.01% of a core at eighteen such items per packet at 30 Hz, which is not worth the code. `extract_ts_klv` runs at ~2.1–2.2 GiB/s, so its second sweep is real but costs milliseconds on inputs this library will see. Both are recommendations to *close* rather than act. The one number worth keeping: gst extraction over identical bytes runs ~36–39 MiB/s, some fifty-odd times slower than the gst-free path — that is demux and pipeline cost, not ours, but it is where extraction time actually goes.
