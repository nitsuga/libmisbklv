...
* **fix: Generate SEI matching survives the encoder DTS-headroom timeline shift (ADR 0033, fork 30 — #33)**: `x264enc` and `avenc_h264` shift their entire output timeline forward by 1000 hours (`gst_video_encoder_set_min_pts(GST_SECOND*60*60*1000)`, unconditional since GStreamer 1.6; `openh264enc` does not opt in), so under `Sei0604::Generate` every frame missed the KLV PTS map by ~1000 h, injected 0 SEI, and the consume-side eviction then wiped every key on frame one — silently, and only on machines with plugins-ugly installed (CI never exercised x264enc). The matcher now detects the shift — a direct miss beyond a 10 s gate retries at `pts − kEncoderDtsHeadroomNs` under the same backward-only 200 ms window — latches the shifted space for the session on success (match and eviction both use the translated key), and leaves agreeing timelines byte-for-byte unchanged. Running-time/segment-space matching was considered and rejected on measured evidence: the live branch's SEGMENT is already rewritten upstream of the probe (`start=3 s` with PTS from 0 on the working openh264enc path), so conversion breaks the correct case. The hermetic lagging-video test now runs once per available encoder asserting 90/90 SEI each (was: first pick only), and CI installs plugins-ugly to close the blind spot. All CTest green including `generate_path`, which passes for the first time on x264enc machines.
