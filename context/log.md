...
* **Result deref, extract-cap floor, view lifetime** (issue #83, F8/F9/F10):
  ...
  (`KlvStream` also checks it itself), rather than a generic `ResourceLimit` on every frame.
...
* **Accepted live video liveness as a last-delivery timestamp**
  ...
  issue #60). Review added real GStreamer `nullopt` coverage, typed duration
  storage, and a precise serialized-query lifetime contract. Release and core
  sanitizer suites pass. Author: openai/gpt-5.
