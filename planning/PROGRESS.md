...
SEI-probe teardown fix on `fix/live-rtp-teardown-probe-uaf-57` (libmisbklv#37, PR #38) — closes the parrot-to-klv#57 use-after-free. `GstInserter::quiesce_to_null()` (from `~GstInserter` and `finish()`) severs the `Generate` SEI pad probes via `remove_sei_probes()` — a blocking `gst_pad_remove_probe`, so after it no callback can touch the freed `VideoCtx` — then takes the pipeline to `GST_STATE_NULL`; `~VideoCtx` severs idempotently too. The severing alone closes the UAF; a bisect showed the earlier bounded NULL-wait was unnecessary for #57, so it was dropped (a non-probe access after an async NULL stays a known, pre-existing residual on parrot-to-klv#57). Validated before/after under Valgrind memcheck with a mid-stream-teardown harness folded into the suite (pre-fix: 50 invalid accesses into a freed `VideoCtx`; fix: 0). ADR 0024 gained a *Teardown: SEI probe lifetime* section (0020 cross-refs it). Validated post-rework: 200-run stressed `live_rtp` rig at baseline flake rate (1 crash / 88-run uncontaminated stretch), suite 28/28 green. Recently landed on `main`: generate-path encoder-shift fix ([ADR 0033](../context/decisions/0033-generate-sei-encoder-timeline-shift.md), #33 — `x264enc`/`avenc_h264` 1000 h `set_min_pts` headroom silently dropped all SEI; matcher now recovers the encoder-adjusted segment), cancellable insert drain ([ADR 0032](../context/decisions/0032-cancellable-insert-drain.md)), and perf fix #27.
