...
Generate-path encoder fix landed ([ADR 0033](../context/decisions/0033-generate-sei-encoder-timeline-shift.md), #33): `x264enc`/`avenc_h264` move output onto a 1000 h minimum (GStreamer `set_min_pts` DTS headroom), which silently produced 0 SEI under `Sei0604::Generate`; the matcher now detects the mismatch and recovers the actual running time from the encoder-adjusted segment, including non-zero source starts. The hermetic Generate test runs every installed encoder, and CI installs plugins-ugly. Before it, insert-drain cancellation ([ADR 0032](../context/decisions/0032-cancellable-insert-drain.md)) gave `finish()`/`close()` a defaulted stop token, and perf fix #27 removed two per-packet copies (`Message::adopt`, `KlvSink` fast path, wrapped buffer push) — byte-exact passthrough preserved throughout.
