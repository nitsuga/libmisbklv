...
static void test_lagging_map_regression() {
  ...
  // Now simulate consumption advancing to 2.5 s — this should prune the old entry.
  {
    ...
    check(ctx.pts_to_sensor_timestamp.size() == 1,
          "consumer eviction after 2.5 s frame leaves 1 entry");
    check(ctx.pts_to_sensor_timestamp.find(0) == ctx.pts_to_sensor_timestamp.end(),
          "entry at 0 evicted after consumption passes tolerance");
  }
  // Demonstrate old producer prune would have failed: simulate its logic.
  {
    ...
    check(old_map.find(0) == old_map.end(),
          "old producer prune would have evicted pts 0 (demonstrates bug)");
  }
}
  ...
  // after consumption finishes, only the tolerance window remains.
  {
    ...
    for (int i = 0; i < kCount; ++i) {
      const std::uint64_t frame_pts =
          static_cast<std::uint64_t>(i) * static_cast<std::uint64_t>(kStepNs);
      if (frame_pts >= kTolNs) {
        const std::uint64_t thr = frame_pts - kTolNs;
        auto it = ctx.pts_to_sensor_timestamp.begin();
        while (it != ctx.pts_to_sensor_timestamp.end() && it->first < thr)
...
inline unsigned u8_ts(std::span<const std::byte> b, std::size_t i) {
  return static_cast<unsigned>(b[i]);
}

static std::vector<std::byte> extract_video_payload(std::span<const std::byte> ts,
                                                    unsigned want_pid) {
  ...
  if (cur.size() >= 9) {
    ...
    if (cur.size() >= 4 && u8_ts(cur, 0) == 0x00 && u8_ts(cur, 1) == 0x00 &&
        u8_ts(cur, 2) == 0x01) {
      std::size_t hdr = 9 + u8_ts(cur, 8);
      if (hdr < cur.size()) out.insert(out.end(), cur.begin() + hdr, cur.end());
    } else {
  ...
  for (std::size_t off = 0; off + kTsPkt <= ts.size(); off += kTsPkt) {
    ...
    if (table_id == 0x00 && pmt_pid == 0) {
      for (std::size_t j = i + 8; j + 4 <= end; j += 4) {
        ...
        if (prog != 0) {
          pmt_pid = ((u8_ts(p, j + 2) & 0x1f) << 8) | u8_ts(p, j + 3);
          break;
        }
      }
      ...
      while (j + 5 <= end) {
        EsInfo es;
        es.stream_type = u8_ts(p, j);
        es.pid = ((u8_ts(p, j + 1) & 0x1f) << 8) | u8_ts(p, j + 2);
        std::size_t esil = ((u8_ts(p, j + 3) & 0x0f) << 8) | u8_ts(p, j + 4);
        for (std::size_t d = j + 5; d + 2 <= j + 5 + esil;) {
          unsigned tag = u8_ts(p, d), len = u8_ts(p, d + 1);
          if (tag == 0x05 && len >= 4 && u8_ts(p, d + 2) == 'K' && u8_ts(p, d + 3) == 'L' &&
              u8_ts(p, d + 4) == 'V' && u8_ts(p, d + 5) == 'A')
            es.klva = true;
          d += 2 + len;
        }
        out.push_back(es);
...
static std::size_t count_sei(const std::vector<std::byte>& payload) {
  ...
  for (std::size_t i = 0; i + kIdLen + 12 <= payload.size(); ++i) {
    ...
    for (std::size_t k = 0; k < kIdLen && hit; ++k)
      hit = static_cast<unsigned>(payload[i + k]) == static_cast<unsigned>(kId[k]);
    if (!hit) continue;
    if (u8_ts(payload, i + kIdLen + 1 + 2) != 0xFF || u8_ts(payload, i + kIdLen + 1 + 5) != 0xFF ||
        u8_ts(payload, i + kIdLen + 1 + 8) != 0xFF)
      continue;
    ++cnt;
    i += kIdLen + 12 - 1;
  }
...
static void test_lagging_pipeline_one_encoder(const char* enc) {
  ...
  const std::string desc = "pipeline:videotestsrc num-buffers=90 timestamp-offset=3000000000 ! "
                           "videoconvert ! video/x-raw,width=320,height=240,framerate=30/1 ! " +
                           std::string(enc) + " ! h264parse";
  ...
  for (int i = 0; i < kFrames; ++i) {
    ...
    if (!r) {
      check(false, (label + " push").c_str());
      std::printf(" push %d failed %d\n", i, (int)r.error());
      break;
    }
  ...
  check(sei_cnt == static_cast<std::size_t>(kFrames),
        (label + " every frame got SEI (no silent loss)").c_str());
  if (sei_cnt != static_cast<std::size_t>(kFrames)) {
    std::fprintf(stderr,
                 "  expected %d SEI, got %zu — indicates KLV pruned before use or a timeline "
                 "mismatch (the bugs this test guards)\n",
                 kFrames, sei_cnt);
  }
  // Also verify KLV byte-exact via extract
  std::vector<std::byte> back;
  auto er = be->extract(
      out, [&](const KlvPacket& kp) { back.insert(back.end(), kp.bytes.begin(), kp.bytes.end()); });
  ...
  if (back.size() != sent.size())
    std::fprintf(stderr, "  KLV size mismatch sent %zu back %zu\n", sent.size(), back.size());
  if (back != sent) {
