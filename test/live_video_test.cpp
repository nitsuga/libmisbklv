...
static std::string pipeline_desc_for(const std::string& enc) {
  if (enc == "x265enc") {
    return "pipeline:videotestsrc is-live=true num-buffers=60 ! videoconvert ! "
...
static GstFlowReturn one_shot_null_fail_sink_chain(GstPad*, GstObject*, GstBuffer* buffer) {
  gst_buffer_unref(buffer);
  return GST_FLOW_OK;
}

static GstStateChangeReturn one_shot_null_fail_sink_change_state(GstElement* element,
  ...
  return GST_ELEMENT_CLASS(one_shot_null_fail_sink_parent_class)->change_state(element, transition);
}

static void one_shot_null_fail_sink_class_init(OneShotNullFailSinkClass* klass) {
  auto* element_class = GST_ELEMENT_CLASS(klass);
  gst_element_class_set_static_metadata(element_class, "One-shot NULL failure sink", "Sink/Testing",
                                        "Fails the first READY-to-NULL transition",
                                        "libmisbklv tests");
  gst_element_class_add_static_pad_template(element_class, &one_shot_null_fail_sink_pad_template);
...
static void one_shot_null_fail_sink_init(OneShotNullFailSink* self) {
  ...
  GstPad* sink = gst_pad_new_from_static_template(&one_shot_null_fail_sink_pad_template, "sink");
  gst_pad_set_chain_function(sink, GST_DEBUG_FUNCPTR(one_shot_null_fail_sink_chain));
...
static void check_live_eos_uses_downstream_lock() {
  ...
  gst_pad_set_event_function(sink, [](GstPad*, GstObject*, GstEvent* event) -> gboolean {
    gst_event_unref(event);
    return TRUE;
  });
  const bool active = gst_pad_set_active(source, TRUE) && gst_pad_set_active(sink, TRUE);
  ...
  auto eos = std::async(std::launch::async, [&] {
    ...
    const bool sent =
        misbklv::detail::push_live_eos_when_idle(source, sink, {}, std::chrono::milliseconds(200));
  ...
  for (std::size_t off = 0; off + kTsPkt <= ts.size(); off += kTsPkt) {
    ...
    if (table_id == 0x00 && pmt_pid == 0) {
      for (std::size_t j = i + 8; j + 4 <= end; j += 4) {
        ...
        if (prog != 0) {
          pmt_pid = ((u8_hevc(p, j + 2) & 0x1f) << 8) | u8_hevc(p, j + 3);
          break;
        }
      }
      ...
      while (j + 5 <= end) {
        EsInfoHevc es;
        es.stream_type = u8_hevc(p, j);
        es.pid = ((u8_hevc(p, j + 1) & 0x1f) << 8) | u8_hevc(p, j + 2);
        ...
        for (std::size_t d = j + 5; d + 2 <= j + 5 + esil;) {
          ...
          if (tag == 0x05 && len >= 4 && u8_hevc(p, d + 2) == 'K' && u8_hevc(p, d + 3) == 'L' &&
              u8_hevc(p, d + 4) == 'V' && u8_hevc(p, d + 5) == 'A')
            es.klva = true;
          d += 2 + len;
        }
        out.push_back(es);
...
static std::vector<std::byte> extract_pes_payload(std::span<const std::byte> ts,
                                                  unsigned want_pid) {
  ...
  if (!cur.empty()) {
    ...
    if (hdr < cur.size()) {
      ...
      if (cur.size() >= 4 && u8_hevc(cur, 0) == 0x00 && u8_hevc(cur, 1) == 0x00 &&
          u8_hevc(cur, 2) == 0x01) {
...
static bool payload_contains(const std::vector<std::byte>& payload, const std::string& needle) {
  ...
  for (size_t i = 0; i + needle.size() <= payload.size(); ++i) {
    bool hit = true;
    for (size_t k = 0; k < needle.size() && hit; ++k)
      hit = static_cast<char>(payload[i + k]) == needle[k];
    if (hit) return true;
  }
  return false;
}
static bool ts_video_payload_contains(std::span<const std::byte> ts, unsigned want_pid,
  ...
  for (std::size_t off = 0; off + kTsPkt <= ts.size(); off += kTsPkt) {
    ...
    for (std::size_t j = 0; j + needle.size() <= avail; ++j) {
      bool hit = true;
      for (std::size_t k = 0; k < needle.size() && hit; ++k)
        hit = static_cast<char>(p[i + j + k]) == needle[k];
      if (hit) return true;
    }
  }
  return false;
...
static bool decode_hevc_frames(const std::string& path, int* out_frames, bool* skipped,
                               std::string* err_msg) {
  *out_frames = 0;
  *skipped = false;
  ...
  {
    const char* decs[] = {"avdec_h265", "openh265dec", "libde265dec",
                          "vah265dec",  "x265dec",     "h265parse"};
  ...
  const std::string desc =
      ...
      "\" ! tsdemux name=demux demux. ! queue ! h265parse ! decodebin ! fakesink name=sink";
  GError* perr = nullptr;
  ...
  *out_frames = count.load();
  ...
  if (is_err) {
    ...
    if (low.find("decoder") != std::string::npos ||
        low.find("could not decode") != std::string::npos ||
        low.find("no available") != std::string::npos || low.find("missing") != std::string::npos ||
        low.find("plug-in") != std::string::npos || low.find("plugin") != std::string::npos) {
      *skipped = true;
      return true;
    }
    return false;
...
int main(int argc, char** argv) {
  ...
  if (argc < 4) {
    std::fprintf(stderr,
                 "usage: live_video_test <synthetic-video.ts> <synthetic-basic.klv> <tmpdir>\n");
    return 2;
  }
  ...
  std::printf("== error cases: unsupported URIs ==\n");
  {
    auto r = be->open_insert(
        {"file:" + tmpdir + "/err-pipeline.ts", false, "pipeline:invalid !", Sei0604::Preserve});
    ...
    // open_insert fails before file creation? For file sink, file may be created but removed.
    // Ensure not exists. allow exists==false or if exists its not ours? Just check not leaked as
    // failure path may delete. No assertion strict.
    (void)p;
  }
  {
    auto r = be->open_insert({"file:" + tmpdir + "/err-http.ts", false,
                              "http://example.com/video.ts", Sei0604::Preserve});
    check(!r && r.error() == Error::Unsupported, "http:// -> Unsupported");
  }
  {
    auto t0 = std::chrono::steady_clock::now();
    auto r = be->open_insert({"file:" + tmpdir + "/err-rtsp.ts", false,
                              "rtsp://127.0.0.1:9/unreachable", Sei0604::Preserve});
    auto elapsed =
  ...
  {
    auto r = be->open_insert({"file:" + tmpdir + "/err-bare.ts", false,
                              "/tmp/does-not-exist-misbklv-xyz123.ts", Sei0604::Preserve});
  ...
  }
  {
    ...
    check(vsrc.kind == misbklv::detail::VideoSourceKind::Rtsp,
          "parse_video_source rtsp ipv6 -> Rtsp kind");
  }
  {
  ...
  std::printf("== Generate for live sources (H.264) ==\n");
  {
    ...
    if (enc.empty()) {
      ...
      auto r = be->open_insert(
          {"file:" + tmpdir + "/err-gen-pipeline.ts", true, pipe, Sei0604::Generate});
      ...
      auto r2 = be->open_insert({"file:" + tmpdir + "/err-gen-rtsp.ts", true,
                                 "rtsp://127.0.0.1:9/unreachable", Sei0604::Generate});
  ...
  std::printf("== pipeline H.265 Generate unstamped (non-H.264 regression) ==\n");
  {
    ...
    if (asan_opts_top && std::strstr(asan_opts_top, "detect_leaks")) {
      std::printf("  SKIP pipeline H.265 Generate: under ASAN (x265 LSAN leak at "
                  "x265_encoder_open, ASAN_OPTIONS=%s)\n",
                  asan_opts_top);
      ...
      if (!f265 || !f265parse) {
        ...
        const std::string pipe = "pipeline:videotestsrc is-live=true num-buffers=90 ! videoconvert "
                                 "! video/x-raw,format=I420,width=320,height=240,framerate=30/1 ! "
                                 "x265enc ! h265parse ! queue";
        ...
        if (!r && r.error() == Error::Unsupported) {
          std::printf(
              "  SKIP pipeline H.265 Generate: open_insert Unsupported (encoder disappeared)\n");
          ...
          if (r) {
            ...
            for (int i = 0; i < 3; ++i) {
              ...
              if (pkt.empty()) {
                check(false, "pipeline H.265 Generate packet build");
                break;
              }
            ...
            if (file_exists_p(out)) {
              ...
              check(back == sent,
                    "pipeline H.265 Generate KLV byte-exact (unstamped, not corrupted)");
              ...
              // with corrupted video because it rides a separate mux pad.
              {
                ...
                for (auto& e : es) {
                  if (e.stream_type == 0x1B || e.stream_type == 0x24) {
                    vtype = e.stream_type;
                    ++nvideo;
                  }
                ...
                check(es.size() == 2 && nvideo == 1 && nklv == 1,
                      "pipeline H.265 Generate PMT video+KLV");
                check(vtype == 0x24,
                      "pipeline H.265 Generate PMT video stream_type is HEVC (0x24)");
                check(vtype != 0x1B, "pipeline H.265 Generate PMT not H.264 (0x1B)");
                if (vtype != 0x24) {
                  std::printf("  FAIL: expected HEVC (0x24) got 0x%02x — H.264 SEI injection would "
                              "flip this\n",
                              vtype);
                }
                ...
                // raw TS, because TS headers split the PES.
                {
                  unsigned vpid = 0;
                  for (auto& e : es)
                    if (e.stream_type == 0x24) vpid = e.pid;
                  ...
                  if (vpid) {
                    ...
                    std::printf("  video ES payload %zu bytes, MISP %s (raw-packet %s)\n",
                                vpayload_sz, found ? "FOUND (corrupt)" : "absent",
                                found_raw ? "FOUND" : "absent");
                  }
                  check(!found,
                        "pipeline H.265 Generate no H.264 SEI (MISPmicrosectime) in video ES");
                  if (found)
                    std::printf("  FAIL: H.264 SEI leaked into HEVC — pre-fix would corrupt\n");
                }
                int frames = 0;
                bool skipped = false;
                std::string derr;
                ...
                if (skipped) {
                  ...
                  if (!ok || frames == 0) {
                    std::printf("  FAIL: HEVC decode failed — corrupted H.265 would error or "
                                "produce 0 frames\n");
                  }
                }
              }
  ...
  {
    auto r =
        be->open_insert({"file:" + tmpdir + "/err-tsdemux.ts", true,
                         "pipeline:udpsrc port=5000 ! tsdemux ! h264parse", Sei0604::Preserve});
    check(!r && r.error() == Error::Unsupported,
          "pipeline:udpsrc ! tsdemux -> Unsupported (dynamic pad not supported)");
  }
  ...
  std::printf("== previously healthy unbounded live closes during a stall ==\n");
  {
    ...
    if (enc.empty()) {
      ...
      if (r) {
        ...
        check(static_cast<bool>((*r)->push(klv.subspan(0, n), 0)), "stalled live push ok");
        std::this_thread::sleep_for(std::chrono::milliseconds(2500));
        auto fr = (*r)->finish();
        check(static_cast<bool>(fr), "previously delivered live branch survives close-time stall");
        check(file_exists_p(out), "stalled live output preserved");
        if (file_exists_p(out))
          check(std::filesystem::file_size(out) > 1000, "stalled live output non-empty");
        std::remove(out.c_str());
      }
    }
  }
  ...
  {
    std::string desc = "pipeline:appsrc is-live=true format=time "
    ...
    if (r) {
      ...
      const auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(
                               std::chrono::steady_clock::now() - t0)
                               .count();
      check(!fr && fr.error() == Error::Backend, "never-ready live finish reports Backend");
  ...
  std::printf("== NULL failure demotes cancellation to Backend ==\n");
  {
    ...
    if (enc.empty()) {
      ...
      std::string desc = "pipeline:videotestsrc is-live=true ! tee name=t "
      ...
      if (r) {
        const size_t n = packet_frame_length(klv);
        check(static_cast<bool>((*r)->push(klv.subspan(0, n), 0)), "NULL-failure live push ok");
        std::stop_source ss;
        ss.request_stop();
        auto fr = (*r)->finish(ss.get_token());
        check(!fr && fr.error() == Error::Backend, "NULL failure overrides cancellation success");
  ...
  std::printf("== file + realtime lift ==\n");
  {
    ...
    if (r) {
      ...
      while (off < klv_bytes.size()) {
        ...
        if (!pr) {
          check(false, "file+realtime push");
          break;
        }
        off += n;
        ++npush;
  ...
  }
  {
    ...
    std::remove(out.c_str());
    auto r = be->open_insert(
        {"file:" + out, true, std::string("file:") + video_file, Sei0604::Preserve});
    ...
    if (enc.empty()) {
      ...
      if (r) {
        ...
        while (off < klv_bytes.size()) {
          ...
          if (!pr) {
            std::printf(" push %zu failed %d\n", npush, (int)pr.error());
            check(false, "pipeline push");
            break;
          }
          off += n;
          ++npush;
  ...
  {
    std::string enc = pick_h264_encoder();
    std::string pipe = enc.empty() ? "pipeline:videotestsrc is-live=true num-buffers=10 ! "
                                     "videoconvert ! video/x-raw,width=320,height=240 ! fakesink"
    ...
    if (s) {
      ...
      g_object_get(s, "ttl-mc", &ttl, "multicast-iface", &iface, "loop", &loop, "auto-multicast",
                   &am, nullptr);
  ...
  std::printf("== hermetic UDP loopback with pipeline live video ==\n");
  {
    ...
    std::string enc2 = []() {
      GstElementFactory* f = gst_element_factory_find("x265enc");
      if (f) {
        gst_object_unref(f);
        return std::string("x265enc");
      }
      return pick_h264_encoder();
    }();
    std::string pipeline_desc =
        enc2.empty() ? "pipeline:videotestsrc is-live=true num-buffers=60 ! videoconvert ! "
                       "video/x-raw,width=320,height=240,framerate=30/1 ! fakesink"
    ...
    if (enc2.empty()) {
      ...
      if (!ins) {
        ...
        // gst_extract for udp ends after idle timeout (default ~? 1-2 sec). gst_stream_nonframing
        // uses 15005 and waits 4 sec overall. rx thread will return after udpsrc idle timeout (~1
        // sec post EOS? Actually EOS not sent over udp, so idle timeout is expected) Wait a bit
        // then join with timeout? Just join (will block until idle timeout). Ins already finished
        // and pipeline NULL, no more udp traffic, so rx should end soon. To avoid indefinite block,
        // join with timed wait? We'll join with 10 sec timeout via polling. Simple: join with
        // std::thread join after 6 sec sleep then check extract_ok may still be waiting but udpsrc
        // idle timeout is 1s default? In gst_extract.cpp it's likely 1 sec. We'll just wait 3 sec
        // after finish then join.
        std::this_thread::sleep_for(std::chrono::milliseconds(2000));
        // If still not ok, need to stop? But extract returns after idle timeout, which should be ~2
        // sec after last packet. Wait for thread with try. Use timed join via polling atomic
        for (int i = 0; i < 30 && !extract_ok.load(); ++i)
        ...
        // Since std::thread join is blocking, we need to ensure extract will terminate. It does via
        // idle timeout 5 sec? Let's wait up to 8 sec. To avoid hanging test, detach with stop? We
        // don't have stop_token here; but extract for udp should timeout on its own after ~? Check
        // gst_extract idle timeout = 2 sec? We'll just join with 10 sec upper bound using async
        // wait.
        auto start = std::chrono::steady_clock::now();
        while (std::chrono::steady_clock::now() - start < std::chrono::seconds(8)) {
          if (extract_ok.load()) break;
          // Check if thread finished? We can't non-blockingly check joinable without join. Use try
          // with sleep.
          std::this_thread::sleep_for(std::chrono::milliseconds(100));
          // If thread is done, extract_ok would be set, but we still need to join to reap.
          // We'll attempt to see if thread is still joinable and do a timed approach via
          // std::async? Simplify: just wait then join blocking with timeout via future. For now,
          // just sleep then join - but join will block until extract returns, which should be <5
          // sec. To avoid infinite hang, we will use a watchdog: if after 8 sec not done, we
          // consider fail and detach? We'll just try to join with timeout by moving thread to
          // detached after 8 sec is not ideal. Let's just join with a detached timer that not
          // needed; the test harness has ctest timeout 600 sec.
          if (extract_ok.load()) break;
        }
        ...
        if (rx.joinable()) {
          ...
          while (!joined.load() &&
                 std::chrono::steady_clock::now() - j0 < std::chrono::seconds(8)) {
            std::this_thread::sleep_for(std::chrono::milliseconds(100));
          }
          if (!joined.load()) {
      ...
      if (enc.empty()) {
        ...
        if (!ins) {
          ...
          if (rx.joinable()) {
            ...
            if (!joined.load()) {
              joiner.detach();
              check(false, "clock pinning rx did not terminate");
            } else
              joiner.join();
          }
        } else {
          ...
          for (size_t i = 0; i < expected_pts.size(); ++i) {
            ...
            if (!pr) {
              std::printf(" clock pinning push %zu failed\n", i);
              check(false, "clock pinning push");
              break;
            }
            off += n;
          ...
          // Wait for receiver idle timeout after EOS (udp has no EOS signaling; idle timeout ends
          // extract). Last PTS is ~3s, pipeline needs ~3s to render it through udpsink sync; wait
          // accordingly.
          ...
          std::vector<std::byte> expected_bytes;
          {
            ...
            for (size_t i = 0; i < expected_pts.size(); ++i) {
              ...
              if (n == 0) break;
              expected_bytes.insert(expected_bytes.end(), klv_bytes.begin() + eo,
                                    klv_bytes.begin() + eo + n);
              eo += n;
            }
          }
          ...
          if (pts_out.size() == expected_pts.size()) {
            ...
            for (size_t i = 0; i < pts_out.size(); ++i) {
              ...
              if (!within) {
                std::printf("  pts pinning drift fail pkt %zu: got %lld ns expected %lld ns "
                            "base_delta %lld ns drift %lld ns\n",
                            i, (long long)pts_out[i], (long long)expected_pts[i],
                            (long long)base_delta, (long long)drift);
                all_within = false;
              }
            ...
            if (pts_out.size() >= 2) {
              for (size_t i = 1; i < pts_out.size(); ++i) {
                ...
                if (idelta >= 100'000'000LL) {
                  std::printf("  interval pinning fail %zu->%zu: got %lld exp %lld\n", i - 1, i,
                              (long long)got_interval, (long long)exp_interval);
                }
              }
              ...
              for (size_t i = 1; i < pts_out.size(); ++i) {
                std::int64_t d =
                    (pts_out[i] - pts_out[i - 1]) - (expected_pts[i] - expected_pts[i - 1]);
                if (d < 0) d = -d;
