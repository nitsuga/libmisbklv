  ...
  void drain() {
    ...
    while (pos < reassembly.size()) {
      ...
      const auto ul = std::search(rest.begin(), rest.end(), kSmpteUlPrefix.begin(),
                                  kSmpteUlPrefix.end());
      if (ul == rest.end()) {
        // Retain the longest possible partial UL so a prefix split across
        // appsink buffers can complete on the next callback.
        std::size_t suffix = 0;
...
// A UDP source that has delivered data but then goes quiet this long is treated
// as ended (no EOS crosses UDP). Longer than the ~33 ms inter-packet gap, so it
// never trips mid-stream; short enough to end the extract promptly (B4).
...
// or a live "udp:HOST:PORT" / "srt:URI". Sets `*udp` only for UDP, whose
// source element reports an idle timeout; SRT has no equivalent natural end.
 ...
 public:
  Result<std::monostate> extract(std::string_view source,
    ...
    if (gst_element_set_state(pipeline, GST_STATE_PLAYING) == GST_STATE_CHANGE_FAILURE) {
      ...
      // Files end at EOS. UDP alone posts GstUDPSrcTimeout after it idles; once
      // any bytes have arrived that is a natural end. SRT has no corresponding
      // idle signal. A finite pop timeout also lets us poll `stop` for
      // cooperative early cancellation (ADR 0019).
